...
// Aangepast voor gebruik met ArduinOS van Tobias de Bildt
...
// Convert the character after the backslash of an escaped character to the
// character
...
int main(int argc, char *argv[]) {
    ...
    while (readToken(file, buf) != EOF) {
        ...
        if (*buf == '\'') {  // char
            ...
            if (command) {
                if (prog[pc - 1] == C_IF || prog[pc - 1] == C_ELSE ||
                    prog[pc - 1] == C_WHILE) {
    ...
    // connect to Arduino
#ifdef _WIN32
    h = CreateFile(argv[2], GENERIC_READ | GENERIC_WRITE, 0, 0, OPEN_EXISTING,
                   0, 0);
    ...
    timeoutParams.ReadIntervalTimeout =
        2;  // wait 2 ms for each character (9600 bps = 1.04 ms per character)
